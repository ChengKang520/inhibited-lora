...
def main():
    ...
    args_p = TrainingArguments(
        ...
        report_to="tensorboard"   # report_to="wandb"   "tensorboard"/
    )
    ...
    output_text = " I put my red bag in the black bag"
    input_text = "What is the colour of my bag"

    output_text = output_text.split()
    input_text = input_text.split()

    attention_scores_plot = torch.zeros(12, len(input_text), len(output_text))

    for i_inputs in range(len(input_text)):
        for i_outputs in range(len(output_text)):
            ...
            for i_plot in range(len(attention_scores_draw)):
                attention_scores = torch.mean(torch.squeeze(attention_scores_draw[i_plot]), 1)
                attention_scores_plot[:, i_inputs, i_outputs] = torch.mean(attention_scores, 1)

    inputs = tokenizer(question, text, return_tensors="pt")

    print('#####################################')
    attention_scores_plot = torch.squeeze(attention_scores_plot).detach().numpy()
    a_size = attention_scores_plot.shape


    plot_ = plt
    fig = plot_.figure()
    print(a_size)
    for i_plot in range(a_size[0]):
        ax = fig.add_subplot(3, 4, (i_plot + 1))
        imgplot = plot_.imshow(attention_scores_plot[i_plot, :, :])
        ax.set_title(i_plot + 1)
    plt.colorbar(orientation='vertical')
    plot_.show()
    # plot_.savefig('attention_score' + str(self.layer_i) + '.png')
    plot_.savefig('attention_score.png')
    ...
    for i_plot in range(a_size[0]):
        plot_0 = plt
        fig = plot_0.figure()
        imgplot = plot_0.imshow(attention_scores_plot[0, :, :])
        ...
        plot_0.show()
        plot_0.savefig('attention_score_layer' + str(i_plot) + '.png')
    print('#####################################')
